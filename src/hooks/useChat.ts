...
export function useChat() {
  ...
  useEffect(() => {
    ...
    const fetchConversations = async () => {
      ...
      const convIds = myParticipations.map(
      ...
      if (convMeta && allParticipants) {
        const formatted: Conversation[] = convMeta.map(
          (c: { id: string; updated_at: string }) => {
            const other = allParticipants.find(
              (p: { conversation_id: string }) => p.conversation_id === c.id,
            ) as
              | {
                  conversation_id: string;
                  user_id: string;
            ...
            return {
              ...
              other_user: other
                ? {
                    id: other.user_id,
                    full_name: Array.isArray(other.profiles)
                      ? other.profiles[0]?.full_name
                      : other.profiles?.full_name,
                    avatar_url: Array.isArray(other.profiles)
                      ? other.profiles[0]?.avatar_url
  ...
  const startConversation = async (otherUserId: string) => {
    ...
    if (error) {
      ...
      alert(
        `Failed to start conversation: ${error.message}. Please make sure the chat SQL schema has been run on Supabase.`,
      );
      return null;
    }

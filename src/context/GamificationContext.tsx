...
export function GamificationProvider({
  ...
  const xpNeededForRank =
    currentRank.maxXp === Infinity
      ? 1000
      : currentRank.maxXp - currentRank.minXp + 1;
  const xpProgress =
    level === 5
      ? 100
      : Math.min((xpInCurrentRank / xpNeededForRank) * 100, 100);
  ...
  const getXpReason = (amount: number): string => {
    switch (amount) {
      case XP_ACTIONS.CREATE_POST:
        return "Đăng bài! 📝";
      case XP_ACTIONS.DAILY_CHECKIN:
        return "Điểm danh! 📅";
      case XP_ACTIONS.RECEIVE_COMMENT:
        return "Nhận Comment! 💬";
      case XP_ACTIONS.RECEIVE_SHARE:
        return "Nhận Share! 🔁";
      case XP_ACTIONS.RECEIVE_LIKE:
      case XP_ACTIONS.GIVE_COMMENT:
        return "Tương tác! ⚡";
      default:
        ...
        (payload: {
          ...
          if (levelGained > 0) {
            setTimeout(() => {
              showXpGain(
                0,
                `🎉 Level Up! You're now Level ${newProfile.level}!`,
              );
            }, 2500);
          }

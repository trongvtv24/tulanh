    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Thao tác này sẽ đặt Level và XP của <b>{userName}</b> về mức khởi
            đầu của Level được chọn.
            <br />
            Hành động này chỉ dành cho Admin.
          </DialogDescription>
        </DialogHeader>
          ...
          >
            {RANKS.map((rank) => (
              <option key={rank.level} value={rank.level.toString()}>
                Level {rank.level} - {rank.nameVi || rank.name} ({rank.minXp}{" "}
                XP)
              </option>
            ))}
          </select>

...
export function CreatePost({
  ...
  const handleSubmit = async () => {
    if ((!content.trim() && !selectedImage && !title.trim()) || isPosting)
      return;
    ...
    <>
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogTrigger asChild>
          <Card
            className={`mb-4 cursor-pointer hover:bg-accent/5 transition-colors ${disabled ? "opacity-60 pointer-events-none" : ""}`}
          >
        ...
        <DialogContent className="sm:max-w-[550px] p-0 gap-0 overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-9 w-9 text-yellow-500 rounded-full hover:bg-yellow-50 dark:hover:bg-yellow-900/20"
          ...
          <DialogFooter className="p-4 border-t bg-background z-10">
            <Button
              ...
              disabled={
                (!content.trim() && !selectedImage && !title.trim()) ||
                isPosting
              }
            >

    ...
    <div className="flex flex-row items-start justify-between space-y-0 p-4 pb-2">
      <div className="flex items-center gap-3">
        ...
        <div>
          <Link
            href={`/profile/${post.user.id}`}
            className="font-semibold text-sm flex items-center gap-2 hover:underline cursor-pointer"
          >
            {post.user.name}
            {post.user.title && (
        ...
        <DropdownMenuContent align="end" className="w-48">
          ...
          {(isAuthor || isAdmin) && (
            ...
            >
              ...
              {isAdmin && !isAuthor ? t.admin.deleteAdmin : t.feed.delete}
            </DropdownMenuItem>
          )}

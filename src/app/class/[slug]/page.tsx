      ...
      if (error || !commData) {
        ...
        if (slug === "youtube-creators") {
          ...
          setCommunity({
            ...
            description:
              "Cộng đồng dành cho những nhà sáng tạo nội dung Youtube.",
            icon: "Youtube",
            required_level: 0,
          });
        }
    ...
    <div className="space-y-6 container max-w-5xl mx-auto pb-10">
      ...
      <Tabs defaultValue="feed" className="w-full mt-8">
        <TabsList>
          ...
          {isAdmin && (
            <TabsTrigger value="admin" className="text-orange-500">
              Admin Dashboard ({pendingMembers.length + pendingPosts.length})
            </TabsTrigger>
          )}
            ...
            <>
              <div className="bg-background rounded-xl border border-border/50 p-4">
                <h3 className="font-semibold mb-4 flex items-center gap-2">
                  Create Post
                  {level < 2 && (
                    <span className="text-xs font-normal text-red-500 bg-red-500/10 px-2 py-0.5 rounded">
                      (Level 2 Required)
                    </span>
                  )}
                </h3>
